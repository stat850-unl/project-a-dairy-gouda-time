...
# dairy_basket_header<-c("Year","RetailPrice_Dollars","FarmValue_Dollars","FarmShare_Percent","","","")
# dairy_basket <- read_csv("dairy_basket.csv",
#                        col_names = dairy_basket_header,
#                        col_types = "dddd_", skip = 3) %>%
#   filter(Year <= 2018) %>%
#   select(-5)
...
average<-AllProducts%>%
  group_by(Year)%>%
  ggplot(aes(x=Year, y=FarmShare_Percent)) +
  geom_line()+
  geom_point()
average

ggplot(data=AllProducts, aes(x=Year, y=FarmShare_Percent, group=Product)) +
  geom_line(aes(color=Product))+
  geom_point()

ggplot(data=AllProducts, aes(x=Year, y=RetailPrice_Dollars, group=Product)) +
  geom_line(aes(color=Product))+
  geom_point()
